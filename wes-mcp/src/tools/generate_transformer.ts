...
/**
 * MCP tool handler for generate_transformer.
 *
 * Assembles the full retro-transformer prompt from the base template,
 * routes it to the selected image generation provider, and returns
 * the result back to Claude Code.
 */
...
/** JSON Schema definition for the MCP tool */
  ...
  inputSchema: {
    ...
    properties: {
      ...
      provider: {
        ...
        enum: ["gemini", "chatgpt"],
        description:
          'Image generation backend. "gemini" (default) uses Google Imagen; "chatgpt" uses OpenAI DALL-E 3.',
      },
      action: {
        type: "string",
